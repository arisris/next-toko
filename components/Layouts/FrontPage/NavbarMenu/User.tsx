...
export default function FrontPageNavbarMenuUser({ session }: { session: SessionContextValue}) {
      ...
      button={
        sessionData?.user?.image ? (
          <div className="min-w-[1.5rem] min-h-[1.5rem] inline-flex items-center gap-1">
            ...
            />
            <strong className="hidden lg:inline-flex truncate">{sessionData.user.name}</strong>
          </div>
        ) : (
